...
main = do
    ...
    let result = run (map getInfo infoList) m
    let crate = getCrate result
    print ("crates " ++  show crate)

    let result2 = run2 (map getInfo infoList) m
...
-- Part 1 --
run :: [[Integer]] -> StackMap -> StackMap
run [] m = m
run ([n,i1,i2]:w) m = run w (iter n i1 i2 m)


-- Perform an update
iter :: Integer -> Integer -> Integer -> StackMap -> StackMap
iter n i1 i2 m = M.insert i1 s1' $ M.insert i2 s2' m
   where (s1',s2') = move n (fromJust $ M.lookup i1 m) (fromJust $ M.lookup i2 m )
...
-- Part 2 --
run2 :: [[Integer]] -> StackMap -> StackMap
run2 [] m = m
run2 ([n,i1,i2]:w) m = run2 w (iter2 n i1 i2 m)


iter2 :: Integer -> Integer -> Integer -> StackMap -> StackMap
iter2 n i1 i2 m = M.insert i1 s1' $ M.insert i2 s2' m
   where (s1',s2') = move2 (fromIntegral n) (fromJust $ M.lookup i1 m) (fromJust $ M.lookup i2 m )

move2 :: Int -> Stack -> Stack -> (Stack, Stack)
move2 n s1 s2 = (s1', s2')
  where (values, s1') = splitAt n s1
        s2' = values ++ s2